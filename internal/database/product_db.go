...
func (pd *ProductDB) UpdateProduct(product *entity.Product) error {
	_, err := pd.db.Exec("UPDATE products SET name = ?, description = ?, price = ?, category_id = ?, image_url = ? WHERE id = ?", product.Name, product.Description, product.Price, product.CategoryID, product.ImageURL, product.ID)
	if err != nil {
		return err
	}
